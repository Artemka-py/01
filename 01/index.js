...
function fill_rows(data, parentId) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (data[i].parentId === parentId && (filter > 1 ? filter = filter : data[i].isActive == filter)) {
            isTrue = true
            row += ` 
            <tr tr_id="${data[i].id}">
                <td>${data[i].id}</td>
                ...
                <td class="acc-modal">
                <div>
                    <button onclick="onOpen(${data[i].id})" type="button" class="btn btn-primary">
                    Child view
                    </button>
                </div>
                </td>   
...
function onOpen(id) {
    let options = {
        title: 'Child elements: ' + restAPI[id - 1].name,
        closable: true,
        content: `
        ...
        <thead>
            <tr class="ths">
                ...
                <th id="ffilter${id}" onclick="Filtered(${id})" data-column="isActive" data-order="desc">isActive</th>
    ...
    let innerOptions = fill_rows(restAPI, id)
